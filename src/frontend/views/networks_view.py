...
class NetworksView(ListView):
  ...
  def get_context_data(self, **kwargs):
      ...
      context["all_runs_to_show"] = []
      show_all_runs = (
        (self.viewing_current_run and Run.objects.count() > 1) or
        (self.viewing_current_run and Run.objects.count() == 1 and self.run is None)
      )
      if show_all_runs:
        context["all_runs_to_show"] = list(Run.objects.all().order_by("-created_at"))
      return context
