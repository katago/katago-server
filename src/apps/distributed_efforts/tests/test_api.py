...
class TestGetSelfplayTask:
    ...
    def test_get_job_authenticated_valid_git_revision1_multipart(self):
        ...
        response = client.post(
            "/api/tasks/", {"git_revision": "abcdef123456abcdef123456abcdef1234567890"}, format="multipart"
        )
        data = copy.deepcopy(response.data)
        data["network"]["created_at"] = None  # Suppress timestamp for test
        data["run"]["id"] = None  # Suppress id for test
...
class TestGetSelfplayTaskForced:
    ...
    def test_get_job_fail(self):
        ...
        response = client.post(
            "/api/tasks/",
            {"git_revision": "abcdef123456abcdef123456abcdef1234567890", "allow_rating_task": False},
            format="multipart",
        )
        data = copy.deepcopy(response.data)
        assert (
...
class TestGetRatingTaskForced:
    ...
    def test_get_job_fail(self):
        ...
        self.r1.save()
        response = client.post(
            "/api/tasks/", {"git_revision": "1111222233334444555566667777888899990000", "allow_selfplay_task": False}
        )
        data = copy.deepcopy(response.data)
        assert (
            str(data)
...
class TestStartPoses:
    ...
    def test_upload_startpos_auth_fail(self):
        ...
        response = client.post(
            "/api/startposes/",
            {"run": "http://testserver/api/runs/testrun/", "data": {"foo": 3, "bar": ["abc"]}, "weight": 2.5},
            format="json",
        )
        data = copy.deepcopy(response.data)
        assert (
    ...
    def test_upload_startpos_success_but_still_no_game2(self):
        ...
        response = client.post(
            "/api/startposes/",
            {"run": "http://testserver/api/runs/testrun/", "data": {"baz": 3, "123": [[(1, 2)]]}, "weight": 4.0},
            format="json",
        )
    ...
    def test_startpos_distribution(self):
        ...
        for i in range(100):
            response = client.post(
                "/api/tasks/",
                {"git_revision": "abcdef123456abcdef123456abcdef1234567890", "task_rep_factor": 4},
                format="multipart",
            )
            assert response.status_code == 200
