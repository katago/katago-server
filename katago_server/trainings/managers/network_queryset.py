...
class NetworkQuerySet(QuerySet):
    def select_best_without_uncertainty(self):
        self.order_by("-log_gamma_lower_confidence").first()

    def select_one_of_the_best_with_uncertainty(self):
        best_networks = self.order_by("-log_gamma_upper_confidence")[:10]
        return random_weighted_choice(best_networks)

    def select_one_of_the_more_uncertain(self):
        more_uncertain_networks = self.order_by("-log_gamma_uncertainty")[:10]
