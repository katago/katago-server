...
class NetworkPdManager(Manager):
    ...
    def bulk_update_rankings_from_dataframe(self, dataframe):
        ...
        dataframe['log_gamma_upper_confidence'] = np.round((dataframe['log_gamma'] + 2 * dataframe['log_gamma_uncertainty']) * 400 * log10(e), decimals=2)
        dataframe['log_gamma_lower_confidence'] = np.round((dataframe['log_gamma'] - 2 * dataframe['log_gamma_uncertainty']) * 400 * log10(e), decimals=2)
